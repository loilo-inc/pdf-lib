...
export async function test() {
  ...
  const [
    ...
    fetchBinaryAsset("fonts/fantasque/OTF/FantasqueSansMono-BoldItalic.otf"),
  ...
  const catRidingUnicornImage = await pdfDoc.embedJpg(catRidingUnicornBase64);
  ...
  const [normalPdfBytes] = await Promise.all([
