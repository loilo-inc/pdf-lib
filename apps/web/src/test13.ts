...
async function test() {
  ...
  const etweImg = await pdfDoc.embedPng(
    await fetchBinaryAsset("images/etwe.png"),
  );
  ...
  const selfDriveImg = await pdfDoc.embedPng(
    await fetchBinaryAsset("images/self_drive.png"),
  );
  ...
  for (const path of pngSuite) {
    const png = await fetchBinaryAsset(`images/pngsuite/${path}`);
